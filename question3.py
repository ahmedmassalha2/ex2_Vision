...
for rotatioOrTranslation in ['rotation', 'translation']:
    #x = 1, y = 2, z = 3
    for axis in [0,1,2]:
        frames = rotationFrames
        if rotatioOrTranslation == 'translation':
            ...
            for i in frame:
                ...
                for u in range(depth.shape[0]):
                    for v in range(depth.shape[1]):
                        ...
                        y = (v - cy) * d / fy
                        camPoint = [x,y,d,1]
                        x_ = newEx.dot(camPoint)
                        newPoint = M.dot(newEx.dot(camPoint))
                ...
                cv.imwrite('Q3-res/'+str(j)+'.png', newIm)
                j+=1
